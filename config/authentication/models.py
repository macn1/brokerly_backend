...
class AccountManager(BaseUserManager):
    ...
    def create_user(
        self, name, email, role="User", password=None, phone_number=None, domain=None
    ):
        if not email:
    ...
    def create_vendor(
        self, name, email, role="Vendor", password=None, phone_number=None, domain=None
    ):
        if not email:
    ...
    def create_superuser(self, name, email, password):
        user = self.create_user(name=name, email=email, password=password, role="Admin")
        user.is_admin = True
        user.is_active = True
        user.is_staff = True
...
class VendorProfile(models.Model):
    ...
    vendor_status = models.CharField(
        max_length=20, choices=VENDOR_STATUS_CHOICES, default="Pending"
    )
